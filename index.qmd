---
title: "Collecting Data: Project 1 by Manan Chitania"
format: 
    html:
        code-fold: true
    pdf: 
...
We measured the voltage at two points in a resistor network to validate the behavior of the circuit. We found the ratio of voltages was: $0.6646\pm 0.0012$ close to the expected value of $0.667$. We also measured the decay over time of the voltage on a capacitor. We used that to estimate the decay time constant and found that to be $\tau = 67.21 \pm 0.2935\mathrm{~ms}$, very close to the expected value of $\tau =66.67\mathrm{~ms}$.
...
Note that these are 5% resistors, we we'd be unsuprised to see a result with a variation of around 5%, or maybe a little more. However the actual result is within 3%, so it's pretty good. 
...
This experiment taught us the basics of microcontrollers. It gave us knowledge of pins on the controller and also showed us an example of the coding we are supposed to do for this course. The results were very close to the sample results which reflects that the experiment was performed right and carefully.
